...
**Example**: To add 247 + 385 using place value blocks, combine 2 hundreds blocks and 3 hundreds blocks to get 5 hundreds, 4 tens blocks and 8 tens blocks to get 12 tens (which regroups as 1 hundred and 2 tens), and 7 ones blocks and 5 ones blocks to get 12 ones (which regroups as 1 ten and 2 ones). The final result is 632.

```
    Addition with Place Value Blocks: 247 + 385 = 632
    ...
    Combined (before regrouping):
    5 hundreds + 12 tens + 12 ones
    
    After regrouping:
    5 hundreds + (1 hundred + 2 tens) + (1 ten + 2 ones)
...
**Example**: When adding 3,567 + 2,849, we start with the ones place (7 + 9 = 16), regroup 10 ones as 1 ten and write 6 in the ones place. In the tens place, we have 6 + 4 + 1 (carried) = 11, so we regroup 10 tens as 1 hundred and write 1 in the tens place. We continue this process through the hundreds and thousands places to get 6,416.

```
    Addition of Multi-digit Numbers: 3,567 + 2,849 = 6,416
    
      1 1 1    ← Regrouping (carrying)
      3,5 6 7
    + 2,8 4 9
    ---------
      6,4 1 6
    ...
    1. Ones place: 7 + 9 = 16
       Write 6, carry 1 to tens place
    
    2. Tens place: 6 + 4 + 1(carried) = 11
       Write 1, carry 1 to hundreds place
    
    3. Hundreds place: 5 + 8 + 1(carried) = 14
       Write 4, carry 1 to thousands place
    
    4. Thousands place: 3 + 2 + 1(carried) = 6
       Write 6
...
**Example**: To subtract 362 - 147 using place value blocks, we need to remove 7 ones from 2 ones, which requires regrouping 1 ten as 10 ones. After regrouping, we have 12 ones, and can remove 7 ones to get 5 ones. Similarly, we need to remove 4 tens from 5 tens (after the previous regrouping), leaving 1 ten. Finally, we remove 1 hundred from 3 hundreds, leaving 2 hundreds. The result is 215.

```
    Subtraction with Place Value Blocks: 362 - 147 = 215
    ...
    Need to subtract 147:
    
    Regrouping needed: 1 ten → 10 ones
...
**Example**: When subtracting 5,734 - 2,946, we start with the ones place. Since 4 is less than 6, we regroup 1 ten as 10 ones, making 14 ones - 6 ones = 8 ones. In the tens place, we now have 2 tens (after regrouping) - 4 tens, which requires regrouping 1 hundred as 10 tens. This gives us 12 tens - 4 tens = 8 tens. We continue this process through the hundreds and thousands places to get 2,788.

```
    Subtraction of Multi-digit Numbers: 5,734 - 2,946 = 2,788
    ...
    1. Ones place: 4 < 6, so regroup
       Borrow 1 ten → 10 ones + 4 ones = 14 ones
       14 - 6 = 8 ones
    
    2. Tens place: 3 - 1(borrowed) = 2 tens
       2 < 4, so regroup
       Borrow 1 hundred → 10 tens + 2 tens = 12 tens
       12 - 4 = 8 tens
    
    3. Hundreds place: 7 - 1(borrowed) = 6 hundreds
       6 < 9, so regroup
       Borrow 1 thousand → 10 hundreds + 6 hundreds = 16 hundreds
       16 - 9 = 7 hundreds
    
    4. Thousands place: 5 - 1(borrowed) = 4 thousands
       4 - 2 = 2 thousands
...
**Example**: When subtracting 5,002 - 1,345, the zeros in 5,002 require multiple regroupings. Since 2 is less than 5, we regroup 1 ten as 10 ones. But there are 0 tens, so we must regroup 1 hundred as 10 tens. But there are 0 hundreds, so we must regroup 1 thousand as 10 hundreds. After all regroupings, we have 4 thousands, 9 hundreds, 9 tens, and 12 ones, allowing us to complete the subtraction to get 3,657.

```
    Subtraction with Zeros: 5,002 - 1,345 = 3,657
    ...
    Step-by-step process with multiple regroupings:
    
    1. Ones place: 2 < 5, need to regroup
       But there are 0 tens to borrow from
    
    2. Look to hundreds place: 0 hundreds
       Need to borrow from thousands
    
    3. Thousands place: 5 thousands
       Borrow 1 thousand → 4 thousands
       That 1 thousand = 10 hundreds
    
    4. Hundreds place: 0 + 10 = 10 hundreds
       Borrow 1 hundred → 9 hundreds
       That 1 hundred = 10 tens
    
    5. Tens place: 0 + 10 = 10 tens
       Borrow 1 ten → 9 tens
       That 1 ten = 10 ones
    
    6. Ones place: 2 + 10 = 12 ones
       12 - 5 = 7 ones
    
    7. Tens place: 9 - 4 = 5 tens
    
    8. Hundreds place: 9 - 3 = 6 hundreds
    
    9. Thousands place: 4 - 1 = 3 thousands
...
**Example**: To add 325 + 298 mentally, we might recognize that 298 is close to 300, so we can add 325 + 300 = 625, and then subtract 2 to get 623. Alternatively, we might decompose the numbers by place value: (300 + 20 + 5) + (200 + 90 + 8) = 500 + 110 + 13 = 623.

```
    Mental Math Strategies
    
    Example 1: Adding 325 + 298
    
    Strategy 1: Using landmark numbers
    298 is close to 300
    325 + 300 = 625
    625 - 2 = 623
    
    Strategy 2: Decomposing by place value
    325 = 300 + 20 + 5
    298 = 200 + 90 + 8
    ...
    Example 2: Subtracting 803 - 547
    
    Strategy: Adding up from subtrahend
    547 + 3 = 550 (added 3)
    550 + 50 = 600 (added 50)
    600 + 200 = 800 (added 200)
    3 + 50 + 200 = 253 + 3 = 256
    
    Therefore, 803 - 547 = 256
```
...
**Example**: To estimate 7,834 + 2,567, we might round to the nearest thousand: 8,000 + 3,000 = 11,000. The exact sum is 10,401, which is reasonably close to our estimate. For 9,215 - 3,768, we might round to the nearest thousand: 9,000 - 4,000 = 5,000. The exact difference is 5,447, which is reasonably close to our estimate.

```
    Estimation in Addition and Subtraction
    
    Example 1: Estimating 7,834 + 2,567
    
    Rounding to nearest thousand:
    7,834 → 8,000
    2,567 → 3,000
    
    Estimated sum: 8,000 + 3,000 = 11,000
    
    Exact sum: 7,834 + 2,567 = 10,401
    
    The estimate (11,000) is reasonably close to the exact sum (10,401)
    
    Example 2: Estimating 9,215 - 3,768
    
    Rounding to nearest thousand:
    9,215 → 9,000
    3,768 → 4,000
    
    Estimated difference: 9,000 - 4,000 = 5,000
    
    Exact difference: 9,215 - 3,768 = 5,447
    
    The estimate (5,000) is reasonably close to the exact difference (5,447)
```
