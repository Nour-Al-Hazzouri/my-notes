...
```
    ...
    Step 2: Add a zero as a placeholder to make place values clearer
    
      3.45
    + 2.70
    ------
    
    Step 3: Add the aligned digits
    ...
    Visual representation with place value columns:
    ...
    Key principle: Always align decimal points to ensure
    digits with the same place value are combined.
```
...
2. Add the digits in each column, starting from the right:
   - Hundredths: 3 + 8 = 11, write 1 and carry 1
   - Tenths: 6 + 4 + 1 (carried) = 11, write 1 and carry 1
   - Ones: 5 + 2 + 1 (carried) = 8
3. The sum is 8.11

```
    Adding Decimals
    
    Example: 5.63 + 2.48 = 8.11
    ...
    Hundredths: 3 + 8 = 11
    Write 1, carry 1 to tenths column
    ...
    Tenths: 6 + 4 + 1 (carried) = 11
    Write 1, carry 1 to ones column
    ...
    Visual representation with regrouping:
...
2. Subtract the digits in each column, starting from the right:
   - Hundredths: 2 - 7 requires borrowing, so 12 - 7 = 5
   - Tenths: 7 (after borrowing) - 4 = 3
   - Ones: 7 - 3 = 4
3. The difference is 4.35

```
    Subtracting Decimals
    
    Example: 7.82 - 3.47 = 4.35
    ...
    Step 2: Subtract the digits in each column, starting from the right
    
    Hundredths: 2 - 7 requires borrowing
    Borrow 1 from tenths: 8 becomes 7, and 2 becomes 12
    12 - 7 = 5
    ...
    Visual representation with borrowing:
...
```
    ...
    Example 1: 4.3 + 1.75 = 6.05
    
    Step 1: Align the decimal points and add zeros as placeholders
    ...
    Key principle: Adding zeros after the decimal point doesn't change the value
    0.5 = 0.50 = 0.500
```
...
-----
```
2. Add the tenths: 8 + 6 = 14 tenths = 1 one and 4 tenths
3. Write 4 in the tenths place and carry 1 to the ones place
4. Add the ones: 3 + 4 + 1 (carried) = 8
5. The sum is 8.4
...
-----
```
2. Subtract the hundredths: 0 - 5 requires borrowing, so 10 - 5 = 5
3. Subtract the tenths: 1 (after borrowing) - 8 requires borrowing from the ones, so 11 - 8 = 3
4. Subtract the ones: 5 (after borrowing) - 3 = 2
5. The difference is 2.35

```
    Adding and Subtracting Decimals with Regrouping
    
    Example 1: 3.8 + 4.6 = 8.4
    ...
    8 + 6 = 14 tenths = 1 one and 4 tenths
    Write 4 in the tenths place and carry 1 to the ones place
    ...
    Example 2: 6.2 - 3.85 = 2.35
    
    Step 1: Align the decimal points and add a zero as a placeholder
    ...
    0 - 5 requires borrowing from tenths
    Borrow 1 from tenths: 2 becomes 1, and 0 becomes 10
    10 - 5 = 5
    ...
    1 (after borrowing) - 8 requires borrowing from ones
    Borrow 1 from ones: 6 becomes 5, and 1 becomes 11
    11 - 8 = 3
    ...
    Visual representation with regrouping across decimal point:
...
```
    ...
    Example 1: 2.75 + 3.25 = 6
    
    Strategy: Look for complementary parts that make whole numbers
    
    Step 1: Recognize that 0.75 + 0.25 = 1.00
    
    Step 2: Regroup the addition
            2.75 + 3.25 = (2 + 3) + (0.75 + 0.25)
                        = 5 + 1
                        = 6
    ...
    Example 2: 5 - 2.75 = 2.25
    
    Strategy: Decompose into easier calculations
    
    Step 1: Subtract the whole number part
            5 - 2 = 3
    
    Step 2: Subtract the decimal part
            3 - 0.75 = 3 - 0.25 - 0.5 = 2.75 - 0.5 = 2.25
    
    Alternative approach:
    5 - 2.75 = 5 - 3 + 0.25 = 2 + 0.25 = 2.25
    
    Other mental math strategies:
    - Use landmark numbers (0.5, 1, 1.5, etc.)
    - Round to nearest whole, then adjust
...
```
    Word Problems with Decimal Addition and Subtraction
    
    Example 1: Recipe Ingredients
    
    Problem: If a recipe calls for 2.5 cups of flour and 1.75 cups of sugar,
             how much flour and sugar are needed in total?
    
    Solution: Add the quantities
              2.5 + 1.75 = 4.25 cups
    ...
    Problem: If a runner completes 3.6 miles in the morning and 2.75 miles
             in the evening, how much farther did they run in the morning?
    
    Solution: Find the difference between the distances
              3.6 - 2.75 = 0.85 miles
    ...
    Types of word problems with decimals:
    - Money (prices, change, savings)
    - Measurements (length, weight, volume)
    - Time (hours with decimal parts)
    - Sports statistics (batting averages, race times)
    - Science data (temperature, precipitation)
```
